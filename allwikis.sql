...
-- Utility: what tables are available per wiki, and how big are thy in total?
CREATE OR REPLACE VIEW allwikis.view_wiki_tablesizes AS
    SELECT pg_class.relname, count(distinct nspname) as num_wikis, 
        sum(pg_class.reltuples) as reltuples, 
        pg_catalog.pg_size_pretty(sum(pg_total_relation_size(pg_class.oid::regclass))) as size 
    FROM pg_class 
    JOIN pg_catalog.pg_namespace n ON n.oid=pg_class.relnamespace 
    WHERE pg_class.relkind IN ('r'::"char", 'm'::"char")
    AND n.nspname IN (SELECT get_wiki_namespaces())
...
-- A utility table to track annual page activity. For every page, a row for 
...
CREATE MATERIALIZED VIEW allwikis.page_years AS
    ...
    SELECT year, wiki, page, (date_part('year', created_at)::int=year) as is_first_year
    FROM allwikis.page_stats
